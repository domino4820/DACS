...
function RoadmapViewInner({
  ...
  const handleCompleteToggleRef =
    useRef<(nodeId: string, completed: boolean) => void>();
  ...
  const onPaneClick = useCallback(
    (event: React.MouseEvent) => {
      if (editMode === "add" && nodeToAdd && reactFlowInstance) {
        ...
        if (isAdmin) {
          addNotification(
            `New course "${nodeToAdd.label}" has been added to the roadmap.`
          );
        }
  ...
  const handleAddCourse = useCallback(
    (courseData: any) => {
      ...
      if (isAdmin) {
        addNotification(
          `New course "${courseData.label}" has been added to the roadmap.`
        );
      }
  ...
  const handleDeleteNode = useCallback(
    (nodeId: string) => {
      ...
      const nodeName =
        nodes.find((node) => node.id === nodeId)?.data.label || "Course";
      ...
      if (isAdmin) {
        addNotification(
          `Course "${nodeName}" has been removed from the roadmap.`
        );
      }
  ...
  useEffect(() => {
    if (nodes.length > 0 && handleCompleteToggleRef.current) {
      ...
      const needsUpdate = nodes.some(
        (node) =>
          node.data.showQuickToggle !== showQuickToggle ||
          !node.data.onQuickToggle
      );
  ...
  useEffect(() => {
    ...
    if (savedStyles) {
      ...
      setEdgeAnimated(
        styles.edgeAnimated !== undefined ? styles.edgeAnimated : true
      );
      setShowQuickToggle(
        styles.showQuickToggle !== undefined ? styles.showQuickToggle : true
      );
    }
        ...
        {isEditing && (
          <>
            ...
            <EditModeToolbar
              ...
              onAddNode={() => setIsAddCourseOpen(true)}
            />
