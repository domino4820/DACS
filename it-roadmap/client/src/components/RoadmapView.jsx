...
const EditorToolbar = ({
  ...
  return (
    <div className="absolute top-4 right-4 z-50 editor-glass-panel p-1">
...
const RoadmapViewInner = forwardRef(
  (
    ...
    const mapSourceHandle = (handle) => {
      ...
      // Use the utility function to fix the handle ID
      const fixedHandle = fixHandleId(handle, "source");

      // Additional check to ensure top-source is correctly formatted
      if (handle === "top" && !handle.includes("-source")) {
        return "top-source";
      }
    ...
    const mapTargetHandle = (handle) => {
      ...
      // Use the utility function to fix the handle ID
      const fixedHandle = fixHandleId(handle, "target");

      // Additional check to ensure top handle doesn't have -source suffix
      if (handle.includes("top") && handle.includes("-source")) {
        return "top";
      }
